...
void CMobA1::Initialize()
{
	...
	CBmpMgr::Get_Instance()->Add_Bmp(L"../Data/Mob/MobA1_stand_left_0,11.bmp", L"MobA1_stand_left_0,11");
	CBmpMgr::Get_Instance()->Add_Bmp(L"../Data/Mob/MobA1_hit_left_0.bmp", L"MobA1_hit_left_0");
	CBmpMgr::Get_Instance()->Add_Bmp(L"../Data/Mob/MobA1_die_left_0,9.bmp", L"MobA1_die_left_0,9");

	m_eCurState = STAND;
	m_tFrame = { 0, 5, 0, 300, GetTickCount() };
	m_pFrameKey = L"MobA1_stand_left_0,11";
}
...
#pragma region 플레이어 스킬에 맞으면 데미지 닳음
	...
	if (m_fHp < 0)
	{
		...
		{
			// 아이템
			CObjMgr::Get_Instance()->Add_Object(OBJ_FIELDITEM, CAbstractFactory<CItem_RedPotion>::Create(m_tInfo.fX, m_tInfo.fY));
		...
		{
			m_eCurState = DIE;
			m_pFrameKey = L"MobA1_die_left_0,9";
	...
	else
	{
		bool bHit =
			CCollisionMgr::CheckObj_Multi(
				...
				CObjMgr::Get_Instance()->Get_TypeObj(OBJ_MOB_A1));

		if (bHit)
		{
			m_eCurState = HIT;
			m_pFrameKey = L"MobA1_hit_left_0";
			m_dwTime = GetTickCount();
...
void CMobA1::Move_Around()
{
	if (m_tInfo.fX <= 600.f)
	{
		m_tInfo.fX += 1.f;
	}
	else if (m_tInfo.fX >= 800.f)
	{
		m_tInfo.fX -= 1.f;
	}
