...
CSkill_frontSlash::CSkill_frontSlash()
...
void CSkill_frontSlash::Initialize()
{
...
#pragma endregion
	...
	if (m_eDir == DIR_LEFT)
	{
		m_pFrameKey = L"beta_skill4_frontSlash_effect_left_012345";
	}

	else if (m_eDir == DIR_RIGHT)
	{
		m_pFrameKey = L"beta_skill4_frontSlash_effect_right_012345";
	}

	m_tFrame = { 0, 5, 0, 100, GetTickCount() };

	CSoundMgr::Get_Instance()->PlaySound(L"Beta_FrontSlash_Use_101000100.mp3", SOUND_EFFECT, 1.f);

	__super::Update_Rect();

}

int CSkill_frontSlash::Update()
{
	if (m_bDead)
		return OBJ_DEAD;
...
void CSkill_frontSlash::Late_Update()
{
...
void CSkill_frontSlash::Render(HDC hDC)
{
...
void CSkill_frontSlash::Release()
{
}
